...
### Added
- **Machine-generated files are skipped by default**: padlock now detects and silently skips source files that declare themselves as machine-generated — Go `// Code generated`, Rust `// @generated` / `//! @generated`, C/C++ `// Generated by` / `/* Generated by`, and protobuf `.pb.h`/`.pb.cc`/`.pb.c`/`.pb.cpp` files. These files are typically produced by code generators (protobuf, gRPC, prost, relay, flatbuffers, …) and their layout is fixed by the generator, not actionable by hand. Use `--include-generated` to opt back in. Resolves multi-minute hangs on codebases that include large generated Go compiler tables (e.g. `opGen.go`, `rewriteAMD64.go` in the Go source tree).
