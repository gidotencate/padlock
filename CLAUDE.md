...
- **`padlock-dwarf`** — Binary analysis backend. Reads DWARF debug info (via `gimli` + `object`), raw BTF sections/files, and PDB files (Windows MSVC debug format, via `pdb` crate) to extract struct layout data from compiled binaries. Produces `padlock-core` IR.
  - **DWARF bitfields**: consecutive bitfield members are grouped into a single synthetic `[a:3|b:5]` field. DWARF4 bitfields carry `DW_AT_data_member_location` (byte offset) and `DW_AT_byte_size` (storage-unit size). DWARF5 bitfields carry `DW_AT_data_bit_offset` (absolute bit offset from struct start, not normalized by gimli — handled with `DW_FORM_data1`/`data2`/`data4`/`data8`/`udata`/`sdata`) and typically omit `DW_AT_byte_size`; the storage-unit size is derived from the bit span covered by the group. `DW_AT_bit_size` encoded as `DW_FORM_implicit_const` arrives as `Sdata` (not normalized). Groups where the size cannot be determined fall back to `uncertain_fields`.
  - **DWARF qualifier tags** (`type_resolver.rs`): `DW_TAG_const_type`, `DW_TAG_volatile_type`, `DW_TAG_restrict_type`, and `DW_TAG_atomic_type` (C11 `_Atomic T`) are all transparent qualifier wrappers — they carry no `DW_AT_byte_size` of their own, so the resolver follows `DW_AT_type` to the inner type. Without `DW_TAG_atomic_type` in this arm it fell to the `_` catch-all which read the absent `DW_AT_byte_size` and sized the field as 0B.
  - **DWARF pointer-like tags** (`type_resolver.rs`): `DW_TAG_pointer_type`, `DW_TAG_reference_type`, and `DW_TAG_rvalue_reference_type` (C++ `T&&`) are all sized as `arch.pointer_size`. Without `DW_TAG_rvalue_reference_type` in this arm, rvalue reference fields were sized as 0B.
  - **DWARF C++ inheritance** (`extractor.rs`): `DW_TAG_inheritance` child DIEs represent C++ base-class subobjects. Each such DIE carries `DW_AT_data_member_location` (the byte offset into the derived struct) and `DW_AT_type` (a ref to the base class DIE). The extractor processes these before `DW_TAG_member` children and emits a synthetic `[BaseName]` opaque field of the correct size and offset. Virtual bases (`DW_AT_virtuality != 0`) are skipped — their offsets require vbtable thunk resolution that can't be statically modelled.
  - **PDB support** (`pdb_reader.rs`): iterates the TPI stream, collects non-forward-reference `Class`, `Union`, and `Enumeration` records, resolves `FieldList` members. Bitfield members (where `field_type` is a `TypeData::Bitfield` record) are grouped the same way as DWARF; unknown storage-unit sizes are added to `uncertain_fields`. `Enumeration` types emit a single `__discriminant` field. Array sizes use `dimensions.last()` (cumulative byte lengths, not element counts). `TypeData::BaseClass` (`LF_BCLASS`/`LF_BINTERFACE`) entries in a FieldList are emitted as synthetic `[BaseName]` opaque fields at their recorded byte offset. `TypeData::VirtualBaseClass` (`LF_VBCLASS`/`LF_IVBCLASS`) entries are skipped — offsets are resolved via vbtable thunks at runtime. Source file/line is not available from PDB type records. Routes to this reader when the input file has a `.pdb` extension.
...
- **`padlock-source`** — Source analysis backend. Parses source files using `syn` (Rust) and `tree-sitter` (C/C++/Go/Zig) to extract struct definitions. Also handles concurrency annotation detection (`concurrency.rs`) and fix generation (`fixgen.rs`).
  ...
  - **Go known external concrete types** (`go.rs`): `is_known_external_go_type` lists qualified types whose sizes are fixed regardless of architecture — `sync.Mutex` (8B/align 4), `sync.RWMutex` (24B/align 8), `sync.Once` (12B/align 4), `atomic.Bool`/`atomic.Int32`/`atomic.Uint32` (4B/align 4), `atomic.Int64`/`atomic.Uint64` (8B/align 8), `atomic.Uintptr` (pointer-size), `atomic.Value` (2×pointer). These are not pushed to `uncertain_fields`. All other qualified cross-package types are still uncertain.
  ...
  - **C/C++ `_Atomic(T)` specifier form** (`c_cpp.rs`): tree-sitter represents the `_Atomic(T)` function-like specifier as an `atomic_type` node (child of `field_declaration`), distinct from the `_Atomic` keyword qualifier which is a `type_qualifier` node. The `parse_field_declaration` dispatcher handles `"atomic_type"` by extracting the inner base type child. The `_Atomic ` prefix is also in the qualifier-strip list in `c_type_size_align` as a safety net for strings that arrive pre-concatenated.
  - **C/C++ `__attribute__((aligned(N)))` on fields** (`c_cpp.rs`): when a field carries a GCC/Clang alignment attribute, tree-sitter wraps the field identifier together with the attribute in an `attributed_declarator` node (direct child of `field_declaration`, not of `field_identifier`). `parse_field_declaration` handles `"attributed_declarator"` by extracting the `field_identifier` (or `pointer_declarator`) and the `attribute_specifier` text. `extract_aligned_from_c_field_text` parses `aligned(N)` / `__aligned__(N)` from that text and feeds the value into `effective_alignas`.
  - **C/C++ anonymous nested union sizing** (`c_cpp.rs`): `parse_anonymous_nested` detects when the nested body is a union (`nested_is_union = true`). Instead of flattening all union variants sequentially into the parent (which gave wrong offsets and total size), it computes `max(variant sizes)` rounded to `max(variant alignments)` and returns a single synthetic `__anon_union<size,align>` field. `c_type_size_align` decodes this prefix at the top of the function and returns `(size, align)` directly. The outer struct then places the union as one correctly-sized opaque field. Individual union member names do not appear in the parent struct's field list.
...
- **Zig struct layout stability**: `extern struct` and `packed struct` have stable, ABI-defined layouts and are analyzed at full severity. Regular Zig structs have implementation-defined field order (the compiler may reorder freely) and are flagged with `is_repr_rust = true`, causing findings to be downgraded one severity level — matching the behaviour for plain Rust `repr(Rust)` structs. The `is_extern` flag is detected but no longer discarded; it drives the `is_repr_rust` assignment (`!is_extern && !is_packed`).
- **Rust lifetime-only generic structs**: structs/enums with only lifetime generic parameters (e.g. `struct Cache<'a>`) are now analyzed normally — lifetime params don't affect layout. Only `GenericParam::Type` and `GenericParam::Const` cause a struct to be skipped.
...
- **`StructLayout::uncertain_fields`** (`ir.rs`): a `Vec<String>` of field names whose type size could not be accurately determined from source alone. Populated by: the **Go frontend** for qualified cross-package types that are not in the `is_known_external_go_type` allowlist (e.g. `driver.Connector`, `io.Reader`); the **Zig frontend** for comptime-only field types (`type`, `anytype`, `comptime_int`, `comptime_float`); the **`resolve_nested_structs` post-pass** in `lib.rs` for `Opaque` fields whose type name was not found among the parsed structs in the same file; the **DWARF extractor** for bitfield groups where `DW_AT_byte_size` is absent. Surfaced in `StructReport::uncertain_fields`, the human-readable summary (as a per-struct note), and JSON output. Serialized as a JSON array, omitted when empty.
