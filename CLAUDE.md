...
- **Generated-file skipping** (`padlock-cli/src/paths.rs`): `is_generated_file(path)` checks extensions (`.pb.h`, `.pb.cc`, `.pb.c`, `.pb.cpp`) and the first 512 bytes for standard markers (`// Code generated` Go official, `// @generated` / `//! @generated` Rust/prost/relay, `// Generated by` / `/* Generated by` C/C++). Called by `collect_layouts` when `include_generated` is `false` (the default). Controlled via `--include-generated` in `FilterArgs`.
