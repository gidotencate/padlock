...
- **`fixgen.rs`** — Generates and applies reorder patches. Two layers:
  - **Source-aware generation** (`generate_*_fix_from_source`): extracts verbatim field "chunks" from the original struct source (attributes, doc-comments, visibility modifiers, annotations, trailing commas) using language-specific splitters (`extract_rust_field_chunks`, `extract_c_field_chunks`, `extract_go_field_chunks`, `extract_zig_field_chunks`). For C/C++, chunks are typed as `CChunk::Field` (reorderable data fields) or `CChunk::Passthrough` (method declarations, access specifiers, and other non-field lines); passthrough chunks are emitted verbatim in their original relative positions so method declarations are never dropped during a fix. Reorders field chunks by the optimal IR field order, preserving all decoration. Used by `apply_fixes_rust`, `apply_fixes_c`, `apply_fixes_go`, `apply_fixes_zig` via `apply_fixes_with_source`.
