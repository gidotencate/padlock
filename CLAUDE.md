...
- **`padlock-source`** — Source analysis backend. Parses source files using `syn` (Rust) and `tree-sitter` (C/C++/Go/Zig) to extract struct definitions. Also handles concurrency annotation detection (`concurrency.rs`) and fix generation (`fixgen.rs`).
  ...
  - **Generics/templates are a fundamental limitation**: Generic Rust structs (`struct Foo<T>`), C++ templates (`template<typename T> struct Foo`), and Zig comptime-generic types cannot be accurately sized from source alone without monomorphisation. These are skipped or use placeholder sizes. Use binary analysis (DWARF/BTF) for exact measurements.
...
- **`StructLayout::uncertain_fields`** (`ir.rs`): a `Vec<String>` of field names whose type size could not be accurately determined from source alone. Populated by the Go frontend for qualified types (e.g. `driver.Connector`) and surfaced in `StructReport::uncertain_fields` and the human-readable summary output as a per-struct note. Serialized as a JSON array, omitted when empty.
